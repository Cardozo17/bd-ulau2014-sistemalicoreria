...
public class VentanaPrincipal extends JFrame implements ActionListener{
	...
	private void initGUI() {
		...
		JButton botonVendedores = new JButton("Vendores");
		config.gridx=0;
		config.gridy=3;
		config.gridheight=1;
		config.gridwidth=1;
		...
		JButton botonProeveedores = new JButton("Proveedores");
		config.gridx=0;
		config.gridy=5;
		config.gridheight=1;
		config.gridwidth=1;
				...
				public void actionPerformed(java.awt.event.ActionEvent evt) {
					if (evt.getSource() == botonClientes) {
						...
						if (evt.getSource() == botonVendedores) {
							VentanaPrincipal.this.botonVendedoresActionPerformed(evt);
						}else{
							if (evt.getSource() == botonInventario) {
								VentanaPrincipal.this.botonInventarioActionPerformed(evt);
							}else{
								if (evt.getSource() == botonProeveedores) {
									VentanaPrincipal.this.botonProeveedoresActionPerformed(evt);
								}else{
									if (evt.getSource() == botonCaja) {
										VentanaPrincipal.this.botonCajaActionPerformed(evt);
									}else{
										if (evt.getSource() == botonSalir) {
											VentanaPrincipal.this.botonSalirActionPerformed(evt);
										}
									}
